...
//cylinder(h = 100, r = 3);
x_end(2);
%translate([-12.5,0,0]) cylinder(h=200, r=6.1,center=true, $fn=12);
%translate([12.5,0,0]) cylinder(h=200, r=6.1,center=true, $fn=12);
//!x_end(0);
//!x_carriage();
...
module x_end(motor = 0) mirror([(motor == 0) ? 1 : 0, 0, 0]) difference() {
	...
	if(outer_smooth)
	{
	translate([-(motor_casing / 2 + rod_size ), 0, (x_rod_spacing + rod_size + 8) / 2])  for(end = [0, 1]) mirror([0, 0, end]) translate([motor_casing / 4 + rod_size / 2, 0, -(x_rod_spacing + rod_size + 8) / 2 - 1]) cylinder(r = bearing_size / 2 - .05, h = bearing_length, $fn = 30);}
